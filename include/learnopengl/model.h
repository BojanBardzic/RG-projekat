...
class Model
{
 ...
 private:
  // loads a model with supported ASSIMP extensions from file and stores the resulting meshes in the
  // meshes vector.
  ...
  Mesh processMesh(aiMesh *mesh, const aiScene *scene)
  {
    ...
    {
      Vertex vertex;
      glm::vec3 vector;  // we declare a placeholder vector since assimp_ uses its own vector class
    ...
    for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
    {
      ...
      for (unsigned int j = 0; j < textures_loaded.size(); j++)
      {
        if (std::strcmp(textures_loaded[j].path.data(), str.C_Str()) == 0)
        {
          ...
          skip = true;  // a texture with the same filepath has already been loaded, continue to
